...
namespace APIConsumer
{
    public class CrudCrud<T>
    {
        ...
        public static List<T> GetAll(string url) // Cambiar el tipo de retorno a List<T>
        {
            using (HttpClient client = new HttpClient())
            {
                var response = client.GetStringAsync(url);
                ...
                var json = response.Result;
                var result = JsonConvert.DeserializeObject<List<T>>(json); // Cambiar el tipo de deserialización a List<T>
                return result;
            }
        }
    }
